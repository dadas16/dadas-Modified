<?php
	...
	if($totals>0)
	{
		...
		mysql_query("CREATE VIEW fcontinue.voirpresence AS SELECT date,idParticipant,nomModul,nomFormateur,SUM(presence_mat_t1) AS presence_mat_t1,SUM(presence_amidi_t1) AS presence_amidi_t1 FROM presence GROUP BY idParticipant,nomModul");
		
		mysql_query("CREATE VIEW fcontinue.sommepresence AS SELECT date,idParticipant AS participantid,nomModul,nomFormateur,presence_mat_t1,presence_amidi_t1, presence_mat_t1 + presence_amidi_t1 AS somme_presence FROM voirpresence");
		
		
		$statement = "sommepresence";
		$statemente = "participant";
		
		$strSQL="SELECT date,participantid,nomModul,nomFormateur,presence_mat_t1,presence_amidi_t1,somme_presence,idParticipant,Nom,Prenom	 FROM {$statemente} RIGHT JOIN {$statement} ON participantid = idParticipant LIMIT {$startpoint} , {$limit}";
...
<div id="main_container">
    ...
    <div class="right_header">Welcome Admin  | <a href="logout.php" class="logout">Se déconnecter</a></div>
...
<table id=\"rounded-corner\" summary=\"2007 Major IT Companies' Profit\">
   ...
   <thead>
	...
	while($data=mysql_fetch_assoc($objQuery)){
