...
define("database_table_name","presence"); 
...
$constructs ="SELECT * FROM ".database_table_name." WHERE idParticipant LIKE '%$search_each%' OR date LIKE '%$search_each%' OR nomModule LIKE '%$search_each%' OR nomFormateur LIKE '%$search_each%'   ";
...
if(!$start)
$start=0; 
$requete="SELECT * FROM ".database_table_name." WHERE idParticipant LIKE '%$search_each%' OR date LIKE '%$search_each%' OR nomModule LIKE '%$search_each%' OR nomFormateur LIKE '%$search_each%'     LIMIT $start, $per_page";
//echo $requete;exit;
    	...
    	<tr>
            ...
            <th scope=\"col\" class=\"rounded\">Présence après midi</th>
				...
				while($runrows = mysql_fetch_assoc($getquery))
				{
													...
													$nom=ucfirst($runrows['nomModule']);
													$prenom=ucfirst($runrows['nomFormateur']);
													$mobile=ucfirst($runrows['presence_mat_t1']);
													$genre=ucfirst($runrows['presence_amidi_t1']);
...
//previous button
if (!($start<=0)) 
echo " <a href='searchH.php?search=$search&submit=Search+source+code&start=$prev'>Prev</a> ";    
...
{
if ($i == $start){
echo " <a href='searchH.php?search=$search&submit=Search+source+code&start=$i'><b>$counter</b></a> ";
}
else {
echo " <a href='searchH.php?search=$search&submit=Search+source+code&start=$i'>$counter</a> ";
}  
$i = $i + $per_page;                 
}
}
...
{
echo " <a href='searchH.php?search=$search&submit=Search+source+code&start=0'>1</a> ";
echo " <a href='searchH.php?search=$search&submit=Search+source+code&start=$per_page'>2</a> .... ";
 
$i = $start;                 
for ($counter = ($start/$per_page)+1; $counter < ($start / $per_page) + $adjacents + 2; $counter++)
{
if ($i == $start){
echo " <a href='searceH.php?search=$search&submit=Search+source+code&start=$i'><b>$counter</b></a> ";
}
else {
echo " <a href='searchH.php?search=$search&submit=Search+source+code&start=$i'>$counter</a> ";
}   
...
echo " <a href='searchH.php?search=$search&submit=Search+source+code&start=$next'>Next</a> ";    
}   
echo "</center>";
...
if($result){
echo "<meta http-equiv=\"refresh\" content=\"0;URL=searchF.php\">";
