...
define("database_table_name","module"); 
...
$constructs ="SELECT * FROM ".database_table_name." WHERE categorie LIKE '%$search_each%' OR nom LIKE '%$search_each%' OR code LIKE '%$search_each%' OR dure LIKE '%$search_each%' OR date LIKE '%$search_each%'  ";
...
if(!$start)
$start=0; 
$requete="SELECT * FROM ". database_table_name." WHERE categorie LIKE '%$search_each%' OR nom LIKE '%$search_each%' OR code LIKE '%$search_each%' OR dure LIKE '%$search_each%' OR date LIKE '%$search_each%'  LIMIT $start, $per_page";
//echo $requete;exit;
   ...
   <thead>
    	<tr>
            ...
            <th scope=\"col\" class=\"rounded\">Date</th>
				...
				{
													$id=$runrows['id_Module'];
													$categorie=$runrows['categorie'];
													$nom=ucfirst($runrows['nom']);
													$code=ucfirst($runrows['code']);
													$dure=ucfirst($runrows['dure']);
													$date=ucfirst($runrows['date']);
													$datefrFin=dateformat($date);
...
//previous button
if (!($start<=0)) 
echo " <a href='searchF.php?search=$search&submit=Search+source+code&start=$prev'>Prev</a> ";    
...
{
if ($i == $start){
echo " <a href='searchF.php?search=$search&submit=Search+source+code&start=$i'><b>$counter</b></a> ";
}
else {
echo " <a href='searchF.php?search=$search&submit=Search+source+code&start=$i'>$counter</a> ";
}  
$i = $i + $per_page;                 
}
}
...
{
echo " <a href='searchF.php?search=$search&submit=Search+source+code&start=0'>1</a> ";
echo " <a href='searchF.php?search=$search&submit=Search+source+code&start=$per_page'>2</a> .... ";
...
echo " <a href='searchF.php?search=$search&submit=Search+source+code&start=$next'>Next</a> ";    
}   
echo "</center>";
