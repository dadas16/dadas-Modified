<?php
	...
	if($totals>0)
	{
		...
		$today = date('Y-m-d');
		$sql="SELECT  distinct nomModule, code, DateDebut, DateFin FROM {$statement} where DateFin > current_date  LIMIT {$startpoint} , {$limit}";
		//echo $sql;exit;
...
Veuillez fermer votre session de travail pour éviter que personne d'autre n'utilise votre compte en se déconnectant par le bouton se déconnecter en haut à droite
    ...
    <h2>Liste des Formations en cours</h2> 
...
<table id=\"rounded-corner\" summary=\"2007 Major IT Companies' Profit\">
   ...
   <thead>
    	<tr>
			...
			<th scope=\"col\" class=\"rounded\">Code du Module</th>
            <th scope=\"col\" class=\"rounded\">Date du Début</th>
            <th scope=\"col\" class=\"rounded\">Date de la Fin</th>
	...
	while($data=mysql_fetch_assoc($resultat)){
													...
													$prenom=ucfirst($data['code']);
													$mobile=ucfirst($data['DateDebut']);
													$date1=dateformat($mobile);
													$genre=ucfirst($data['DateFin']);
													$date2=dateformat($genre);
...
<!--<script>
    ...
    <!-- <a href="#" class="bt_blue"><span class="bt_blue_lft"></span><strong>View all items from category</strong><span class="bt_blue_r"></span></a>
     <a href="#" class="bt_red"><span class="bt_red_lft"></span><strong><input name="supprimerb" type="submit" id="supprimerb" onclick="return(confirm('Voulez-vous vraiment supprimer ce(s) participant(s)?'));" value="SUPPRIMER TOUT"></strong><span class="bt_red_r"></span></a> -->
    	</ form>
