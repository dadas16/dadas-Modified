	...
	if (array_key_exists ("submit",$_POST)){ 	
		foreach($_POST as $key=>$value){
			...
			//CHECK DATE 
					if ($_POST['date'] < $today )
					{ 
						$message.=" La date de début ne peut pas être antérieure à celle d'aujourd'hui <br/>";
					}		
					if ($_POST['date1'] < $today )
					{ 
						$message.=" La date de Fin ne peut pas être antérieure à celle d'aujourd'hui <br/>";
					}		
...
Veuillez fermer votre session de travail pour éviter que personne d'autre n'utilise votre compte en se déconnectant par le bouton se déconnecter en haut à droite
    ...
    <div class="right_content">            
     ...
     <h2>Formulaire d'Inscription  d'un Participant à une Formation</h2>
